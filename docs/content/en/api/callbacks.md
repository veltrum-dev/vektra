...
1. Standard `on_click`, `on_change`, `on_focus`, and `on_blur` forms receive [`App`](./gpui-types#app)-level callbacks and suit logic that does not need host Entity state.
2. `on_click_in`, `on_change_in`, `on_focus_in`, and `on_blur_in` receive a `&Context<T>`. They use [`Context::listener`](./gpui-types#contextt) internally, so handlers can mutate `&mut T` and call `cx.notify()`.
...
[`Changeable<T>`](./changeable) uses `on_change` for a user-requested next controlled value. It carries no `ClickEvent` and does not imply that the component committed state.
