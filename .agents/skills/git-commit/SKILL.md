---
name: git-commit
description: 在 Nexora workspace 中准备、审查或创建 Git 提交时使用。强制使用 Conventional Commits v1.0.0 格式，提交内容使用中文，并要求正文使用 Markdown 格式详细说明改动内容、目的、影响范围和验证情况。
---
...
使用这个 skill 为 Nexora workspace 创建清晰、可审阅的 Git 提交。提交信息必须遵循 Conventional Commits v1.0.0，同时人类可读的摘要使用中文，正文使用中文 Markdown。
...
每次提交都必须包含详细的中文 Markdown 正文。不要创建只有一行摘要的提交。
...
如果使用提交信息文件而不是多个 `-m` 参数，并且正文包含 `#` Markdown 标题，使用 `git commit -F <file> --cleanup=verbatim`，避免 Git cleanup 把标题行当成注释移除。

## 正文内容

正文要稍微详细，但不要变成流水账。正文必须是 Markdown，按目的和影响组织内容，不要逐行罗列文件变更。
...
- 段落标题使用 `## 标题`，不要使用 `标题：` 形式。
- 各段落内容优先使用无序列表；命令、路径、crate、scope 和配置键使用反引号。
- 验证命令优先写成列表项，命令本身使用反引号；命令较长时可以使用 fenced code block。
- 不要为了填满模板写空段落；确实没有影响或验证时，明确写出原因。
...
2. 正文存在，并且使用中文 Markdown。
3. 正文使用 `## 改动内容`、`## 目的`、`## 影响范围` 和 `## 验证` 组织内容。
