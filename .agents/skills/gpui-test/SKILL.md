...
name: gpui-test
description: >-
  用于在 Vektra 中编写、调试或复现确定性 GPUI 测试，包括 gpui::test 参数、
...
先检查 Vektra 锁定的 GPUI revision、feature 和现有测试。GPUI 测试宏参数、上下文类型及调试环境变量可能随 revision 变化；只使用当前源码或编译器确认存在的能力，不从旧项目配置推断。

## `#[gpui::test]` 的作用

`#[gpui::test]` 会展开为普通的 Rust `#[test]`，因此可以通过 `cargo test` 执行；只有仓库已经配置 `cargo nextest` 时才使用 nextest。
...
- `ZED_HEADLESS=1`：部分 GPUI 上游 revision 保留的环境变量名，用于让平台推断倾向无头模式。只有当前锁定 revision 或仓库测试实际读取它时才设置。
