//! Vektra GPUI 组件库。
//!
//! 第一阶段提供默认主题、Button、Icon 和 IconButton。Vektra 是组件库，不要求
...
/// Vektra 自带资源。
///
/// 传给 GPUI 原生 `with_assets` 后即可加载 Vektra 默认主题资源。启用 `icons`
/// feature 时，还可加载 `icons/settings.svg` 等内置 SVG 图标。应用有自己的资源源时，
/// 可使用 `Assets::with_overrides(AppAssets)` 组合为单个 GPUI 资源源。
pub mod assets {
