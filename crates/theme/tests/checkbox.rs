...
fn themes_without_checkbox_extension_use_semantic_fallbacks() {
    ...
    let unchecked = theme.checkbox_state("unchecked", "normal").unwrap();
    assert_eq!(unchecked.box_background, theme.semantic.background);
    assert_eq!(unchecked.border, theme.semantic.input_border);
